...
- 🖼️ **預覽本地圖片與下載**：自動偵測 `raw_images` 資料夾內的圖片並載入為預覽背景，調校完畢後可「一鍵下載」PNG 預覽圖。
...
- **⬇️ 下載預覽圖**：提供專屬按鈕，快速將目前的畫布排版結果另存成 PNG 圖片（以標題命名）
- 顯示即時資訊：字級、行數、邊距、浮水印狀態。採用水平單列緊湊顯示，保留更多垂直空間讓畫布能以完整的 16:9 比例展示
